...
/**
 * This abstract class represents a AbstractClient, or Player, that can connect
 ...
 * Each player in the game will have an associated AbstractClient object,
 ...
 * be defined to say how the AbstractClient should respond when a message is
 * received.  Subclasses might also define the closedByError(),
 * serverShutdown(), ClientConnected(), and ClientDisconnected() methods.
...
abstract public class Client implements Serializable{
   ...
   /**
    * this method tells us whether the clients connection to the server is
    * closed
    * @return true or false depending on whether the connection is closed or not 
    */
    ...
    */
   private  class ConnectionToHub implements java.io.Serializable {
      ...
      ConnectionToHub(String host, int port) throws IOException {
         ...
         clientExecutor.execute(new ReceiveThread());
         clientExecutor.execute(new SendThread());
            ...
            try {
               while ( ! closed ) {
                  ...
                  if (obj instanceof DisconnectMessage) {
                     ...
                     if (msg.connecting)
                        ClientConnected(msg.playerID);
                     else
                        ClientDisconnected(msg.playerID);
                  }
                  else
                     messageReceived(obj);
               }
