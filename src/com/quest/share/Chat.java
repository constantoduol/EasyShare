 ...
 *  are notified of the new client.  (The clientIsConnected() method in
 ...
 *  that just disconnected.  (The clients' clientIsDisconnected() method
 *  is called.)</li>
...
public class Chat {
   ...
   private static ConcurrentHashMap<Integer,Socket> allClientSockets=new ConcurrentHashMap();
      // Accepts connections on serverSocket
        ...
        public void run() {
            while (true) {
               try {
                  ...
                  Chat.this.messageReceived(msg.playerConnection, msg.message);
               }
               catch (Exception e) {
   ...
   /**
    * this method returns a servers socket
    */
    ...
    * Note that getClientList() can be called to get a list
   ...
   public Socket getClientSocket(int clientID){
      return allClientSockets.get(clientID);
   }
    ...
    * @param playerID the ID number of the new player.
    */
   protected void clientIsDisconnected(int playerID) {
   }
   ...
   synchronized private void clientDisconnected(int playerID) {
      if (playerConnections.containsKey(playerID)) {
         ...
         StatusMessage sm = new StatusMessage(playerID,false,getClientList());
         sendToAll(sm);
         clientIsDisconnected(playerID);
         allClientSockets.remove(playerID);
   ...
   private class ServerThread implements Runnable {  // Listens for connection requests from clients.
        @Override
      public void run() {
         try {
            while ( ! shutdown ) {
               ...
               ConnectionToClient conn = new ConnectionToClient(incomingMessages,connection);
            }
         }
         catch (Exception e) {
            if (shutdown)
   ...
   private class ConnectionToClient { // Handles communication with one client.
      ...
      ConnectionToClient(BlockingQueue<Message> receivedMessageQueue, Socket connection)  {
         ...
         allClientSockets.put(playerID, connection);
         executor.execute(new SendThread());
      }
         ...
         public void run() {
            ...
            try {
               ...
               if (! closed) {
                  ...
                  System.out.println("Unexpected error shuts down hub's send thread:");
               }
            }
         }
      }
